...
{{- /* Collapse Recharge's 4xx error shapes into one message: a bare string,
       a map of field -> "message", or a map of field -> ["message", ...]. A charge that can't be
       refunded (already refunded, or processed through Shopify's checkout) surfaces here as a 4xx. */ -}}
{{- $errorMessages := "" -}}
{{- if not $isSuccess -}}
...
{{- if $isSuccess -}}
    ...
    {{- /* Guard the nested customer: a charge can come back without it; an unguarded
           .customer.id deref would EXEC-error and fail the action after the refund already landed. */ -}}
    {{- if $charge.customer -}}
