...
{{- else if .rawData.data.subscriptionUpdatePaymentMethod -}}
{{- $payload := .rawData.data.subscriptionUpdatePaymentMethod -}}
{{- /* This mutation's selection has no `ok` field, so Skio's payload cannot supply one
       and reaching here IS the success signal. The output type documents `ok`, and every
       failure path above sets `ok: false`, so leaving it absent on success would have an
       automated caller branching on `ok == true` read a completed change as a failure.
       Merge it in rather than replacing the payload - the payload's own fields still
       have to reach the caller. */ -}}
{{- toJson (merge (dict "ok" true) $payload) -}}
{{- else -}}
{{- fail "Unexpected response structure from Skio API for subscriptionUpdatePaymentMethod" -}}
{{- end -}}
