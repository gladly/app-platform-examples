...
{{- range $s := (default list .data.subscriptions) -}}
    ...
    {{- if and (eq $status "CANCELLED") (ne $context "PERMANENTLY_CANCELLED") -}}
        {{- $name := "Subscription" -}}
...
{{- if gt (len $options) 0}}
    ...
    },
    {
      ...
      "attr": "newBillingDate",
      "input": {"type": "text", "placeholder": "2026-10-15", "optional": false},
      "hint": "Skio requires a date for a reactivated subscription. Agree it with the customer first."
    }
{{- else}}
    {
      "type": "text",
      "text": "This customer has no cancelled Skio subscriptions that can be reactivated. A subscription shown as PERMANENTLY_CANCELLED cannot be brought back."
    }
{{- end}}
  ]
}
