...
{{- $adjustment := .rawData.credit_adjustment -}}

{{- /* Stringify int64 IDs so GraphQL ID! serialization keeps full precision. */ -}}
{{- if $isSuccess -}}
    {{- $formattedIds := dict
            "id" ($adjustment.id | int64 | toString)
            "credit_account_id" ($adjustment.credit_account_id | int64 | toString)
    -}}
    {{- $adjustment = mergeOverwrite $adjustment $formattedIds -}}
{{- end -}}

{
    "error": {{- ($isSuccess | ternary nil $rechargeError) | toJson -}},
    "credit_adjustment": {{- ($isSuccess | ternary $adjustment nil) | toJson -}}
}
