{{- /* This action returns a LIST, so there is nowhere to put an error envelope: an
       ...
       reads "no results" during a Skio outage concludes the customer has no Skio account:
         ...
         2. any response that does not carry a ProductVariants LIST at all - an HTTP 500 or 429
            whose body has no `errors` array, a `"data": null`, a null ProductVariants.
...
{{- end -}}
{{- if gt (len $parts) 0 -}}
    {{- fail (printf "Skio returned an error searching product variants: %s" (join "; " (uniq $parts))) -}}
{{- end -}}
{{- $data := dict -}}
{{- with .rawData.data -}}
    {{- if kindIs "map" . -}}{{- $data = . -}}{{- end -}}
{{- end -}}
{{- $results := get $data "ProductVariants" -}}
{{- if not (kindIs "slice" $results) -}}
    {{- fail (printf "Skio returned a response the Skio app could not read searching product variants: no ProductVariants list in the payload, and no errors array explaining why. Skio's raw response was: %s" (trunc 500 (toJson .rawData))) -}}
{{- end -}}
{{- toJson $results -}}
