...
{{- else -}}
{{- /* The mutation selects the stored note back, so the echo is the end state: text means the
       note was saved, empty (or null) means the subscription now has no note. A blank field
       reaches Skio as an empty string and clears the note, so an empty echo from this form is
       a clear, not a no-op. */ -}}
{{- $note := .action.result.note | default "" | toString -}}
{{- $message := "Subscription note cleared." -}}
{{- $detailText := "The subscription no longer has a note." -}}
{{- if ne $note "" -}}
    {{- $message = "Subscription note saved." -}}
    {{- $detailText = printf "The subscription note is now: %s" $note -}}
{{- end -}}
{
