{{- if and .response (ne .response.statusCode 200) -}}
  ...
  {{- if eq .response.statusCode 400 -}}
    {{- if and .rawData.subscription -}}
      ...
      {{- end -}}
      {{ stop (printf "reactivate failed: %s" $messages) }}
      {{- end -}}
    {{- end -}}
  {{- end -}}
  {{ fail (printf "reactivate failed: %d %s" .response.statusCode .response.body) }}
{{- end -}}
{
  ...
  "iteration": {{if and $iteration (ne $iteration "")}}"{{$iteration}}"{{else}}null{{end}},
  "sequence": {{if and $sequence (ne $sequence "")}}"{{$sequence}}"{{else}}null{{end}},
