...
{{- if ne .response.statusCode 200 -}}
    {{- fail (printf "Magento API error: %s (status %d)" .response.status .response.statusCode) -}}
{{- end -}}
{{- $statusMap := dict
    "complete" "fulfilled"
    "completed" "fulfilled"
...
-}}
{{- with .rawData -}}
{{/* Map the raw Magento order status to a normalized value for order items.
     The order-level status is passed through as-is since Magento supports custom statuses,
...
-}}
{{- if .payment -}}
    {{- $_ := set $result "paymentMethod" .payment.method -}}
{{- end -}}
{{- if .coupon_code -}}
    {{- $_ := set $result "couponCode" .coupon_code -}}
{{- end -}}
{{/* Build order items */}}
{{- $orderItems := list -}}
{{- range .items -}}
...
{{- if .billing_address -}}
    ...
    {{- end -}}
    {{- $_ := set $result "billingAddress" $addr -}}
{{- end -}}
{{- toJson $result -}}
{{- end -}}
