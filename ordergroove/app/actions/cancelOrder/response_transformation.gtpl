...
{{- if and .response (ne .response.statusCode 200) -}}
  ...
  {{- if eq .response.statusCode 400 -}}
    {{- if .rawData.detail -}}
      {{ stop (printf "Cancel order: %s" .rawData.detail) }}
    {{- end -}}
    {{- range $field, $msgs := .rawData -}}
      {{- if kindIs "slice" $msgs -}}
        {{- if gt (len $msgs) 0 -}}
          {{ stop (printf "Cancel order: %s" (join ", " $msgs)) }}
        {{- end -}}
      {{- end -}}
    {{- end -}}
    {{ stop "Cancel order: Ordergroove rejected the change." }}
  {{- end -}}
  ...
  {{- if ge .response.statusCode 500 -}}
    {{ stop "Cancel order: Ordergroove would not change this order. It is usually because the order is no longer open - it may have been placed, rejected or skipped since this card was loaded. Refresh the customer's profile to see the current orders, then try again." }}
  {{- end -}}
  {{ fail (printf "Cancel order failed: %d %s" .response.statusCode .response.body) }}
{{- end -}}
