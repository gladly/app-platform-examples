{{/* Error handling mirrors create_one_time_customer_discount:
...
{{- /* When Shopify reports top-level errors the body carries `"data": null`, so
       `.rawData.data.draftOrderCreateFromOrder` must only be dereferenced after the
...
{{- else -}}
{{- $payload := .rawData.data.draftOrderCreateFromOrder -}}
...
{{- else -}}
    ...
    {{- /* Verify the created draft is attributed to the required customer. A mismatch does NOT
           fail — draftOrderCreateFromOrder already created the draft (it copies the source
           order's purchasing entity and has no customer argument), so we still return the draft
           and attach a non-fatal alert. Capture the match from the raw purchasingEntity before
           it's reshaped below. */ -}}
    ...
    {{- /* Build alerts: empty on a customer match; otherwise a single non-fatal
           customer_mismatch alert (the draft is returned either way). Two cases: the draft
           belongs to a different customer, or it has no customer at all (e.g. a B2B/company
           source order). */ -}}
    {{- $alerts := list -}}
    {{- if not $customerMatches -}}
        {{- $message := "" -}}
        {{- if $draftHasCustomer -}}
            {{- $message = "The draft order was created, but it belongs to a different customer. Verify this was the right order to duplicate." -}}
        {{- else -}}
            {{- $message = "The draft order was created, but it has no associated customer." -}}
        {{- end -}}
