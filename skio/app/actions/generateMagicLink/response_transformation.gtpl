...
{{- else if .rawData.data.generateMagicLink -}}
{{- $payload := .rawData.data.generateMagicLink -}}
{{- if eq (printf "%v" $payload.ok) "false" -}}
    {{- /* generateMagicLink reports its own failure in `error`, not `message` - for example
           when no storefront user matches the email. */ -}}
    {{- $detail := $payload.message | default $payload.error | default "Skio rejected the change." | toString -}}
    {{- /* Four Skio errors send the caller to the wrong action. Rewrite exactly these and
           pass everything else through verbatim - a wrapper that rewords every error hides
           the useful ones. See "Error translation" in the app README. */ -}}
...
{{- end -}}
{{- else -}}
{{- fail "Unexpected response structure from Skio API for generateMagicLink" -}}
{{- end -}}
