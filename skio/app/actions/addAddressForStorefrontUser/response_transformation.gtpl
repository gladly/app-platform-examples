...
{{- else if .rawData.data.addAddressForStorefrontUser -}}
{{- $payload := .rawData.data.addAddressForStorefrontUser -}}
{{- toJson (merge (dict "ok" true) $payload) -}}
{{- else -}}
{{- fail "Unexpected response structure from Skio API for addAddressForStorefrontUser" -}}
{{- end -}}
