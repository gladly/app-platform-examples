...
{{- else if .rawData.data.updateSubscriptionShippingAddressV2 -}}
{{- $payload := .rawData.data.updateSubscriptionShippingAddressV2 -}}
{{- if eq (printf "%v" $payload.ok) "false" -}}
    ...
    {{- end -}}
{{- toJson (dict "ok" false "errorMessage" $detail) -}}
{{- else -}}
{ "ok": true }
{{- end -}}
{{- else -}}
{{- fail "Unexpected response structure from Skio API for updateSubscriptionShippingAddressV2" -}}
{{- end -}}
