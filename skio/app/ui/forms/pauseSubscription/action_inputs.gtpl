{{- /* Form attrs are always strings on the wire, so Int/Float inputs convert here.
       ...
       only one of the two callers, because Gladly AI and Team Assist call pauseSubscription
       ...
       the problem as a missing value or, with no unit chosen, quietly apply an open-ended
       pause nobody asked for.

       A blank field is still omitted entirely, so blank keeps meaning blank and leaving both
       halves empty still means an open-ended pause. */ -}}
{{- $out := dict -}}
