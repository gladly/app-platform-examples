...
{{- if and .response (ne .response.statusCode 200) -}}
  ...
  {{- if eq .response.statusCode 400 -}}
    {{- if .rawData.detail -}}
      {{ stop (printf "%s: %s" "Change prepaid renewal behavior" .rawData.detail) }}
    {{- end -}}
    {{- range $field, $msgs := .rawData -}}
      {{- if kindIs "slice" $msgs -}}
        {{- if gt (len $msgs) 0 -}}
          {{ stop (printf "%s: %s" "Change prepaid renewal behavior" (join ", " $msgs)) }}
        {{- end -}}
      {{- end -}}
    {{- end -}}
    {{ stop (printf "%s: Ordergroove rejected the change." "Change prepaid renewal behavior") }}
  {{- end -}}
  {{ fail (printf "%s failed: %d %s" "Change prepaid renewal behavior" .response.statusCode .response.body) }}
{{- end -}}
