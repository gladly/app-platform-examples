...
{{- if and .response (ne .response.statusCode 200) -}}
  ...
  {{- if eq .response.statusCode 400 -}}
    {{- if .rawData.detail -}}
      {{ stop (printf "%s: %s" "Change frequency" .rawData.detail) }}
    {{- end -}}
    {{- range $field, $msgs := .rawData -}}
      {{- if kindIs "slice" $msgs -}}
        {{- if gt (len $msgs) 0 -}}
          {{ stop (printf "%s: %s" "Change frequency" (join ", " $msgs)) }}
        {{- end -}}
      {{- end -}}
    {{- end -}}
    {{ stop (printf "%s: Ordergroove rejected the change." "Change frequency") }}
  {{- end -}}
  {{ fail (printf "%s failed: %d %s" "Change frequency" .response.statusCode .response.body) }}
{{- end -}}
